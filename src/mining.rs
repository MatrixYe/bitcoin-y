...
fn create_new_block(state: NodeState, pub_key: PubKey) -> Result<Block, MiningError> {
    // 创建待选区块
...
pub fn start_mine(mut state: &NodeState, pub_key: PubKey) -> Result<(), MiningError> {
    // 获取最佳前序区块
    let best_index = state.chain.best_index().ok_or(MiningError::BestIndexNotFound)?;
...
fn get_block_value(total_fee: u64, height: u32) -> u64 {
    total_fee + (INIT_SUBSIDY >> height / 210000)
}
